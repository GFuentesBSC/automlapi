...
def insert_document(uploadDate, filename, extension, phase, uri, nPages, training, dataset_id, uploadMethod_id):
	query = f'INSERT INTO neuralplatform_document(uploadDate, name, extension, phase, uri, nPages, training, dataset_id, uploadMethod_id) ' + \
			f'VALUES ("{uploadDate}", "{filename}", "{extension}", "{phase}", "{uri}", {nPages}, {training}, {dataset_id}, {uploadMethod_id});'
...
def get_pending_and_unblocked_steps():
    ...
    if len(pending_steps) > 0:
        ...
        for pending_stepDefinition in pending_stepDefinitions:
            ...
            if not run_exists(f'SELECT id FROM neuralplatform_step WHERE status NOT IN ("succeeded", "error") AND stepDefinition_id = {sd_dependency}'):
